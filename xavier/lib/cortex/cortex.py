...
class Cortex(Dispatcher):
    ...
    def handle_result(self, recv_dic):
        ...
        if req_id == HAS_ACCESS_RIGHT_ID:
            ...
            if len(self.headset_list) == 0:
                raise Exception("No headset available. Please turn on a headset.")
            elif self.headset_id == '':
                self.headset_id = self.headset_list[0]['id']
                self.query_headset()
            elif not found_headset:
                warnings.warn(
                    "Can not found the headset {}. Please make sure the id is correct.".format(self.headset_id))
            elif found_headset:
                if headset_status == 'connected':
                    ...
                    warnings.warn('query_headset resp: Invalid connection status {}'.format(headset_status))
            ...
            for stream in result_dic['success']:
                ...
                print('The data stream {} is subscribed successfully.'.format(stream_name))
            ...
            if name is None:
                ...
                print('get_current_profile: no profile loaded with the headset {}'.format(self.headset_id))
            ...
            for record in result_dic['failure']:
                ...
                print('export_record resp failure cases: {} : {}'.format(record_id, failure_msg))
            ...
            print('No handling for response of request {}'.format(req_id))
    ...
    def create_session(self):
        if self.session_id != '':
            warnings.warn("There is existed session {}".format(self.session_id))
            return
